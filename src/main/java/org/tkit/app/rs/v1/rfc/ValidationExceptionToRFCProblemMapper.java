...
public class ValidationExceptionToRFCProblemMapper implements ExceptionMapper<ValidationException> {
    ...
    public Response toResponse(ValidationException validationException) {
        ...
        log.error("REST exception URL:{},ERROR:{}", uriInfo.getRequestUri(), validationException.getMessage());
        log.error("REST exception error!", validationException);

        return createResponse(validationException);
    }
